...
enum SurveyFeedbackRequest {
   ...
   */
    ...
    /**
     ...
     If query isn't successful, returns (false, .failureResponse) or (false, .noResponse)
   */
    @discardableResult private static func create(invokeErrorManager: Bool, forBody: [String: Any?], completionHandler: @escaping (Bool, ResponseStatus, HoundError?) -> Void) -> Progress? {
        ...
        let body: [String: Any?] = [ KeyConstant.surveyFeedback.rawValue: forBodyWithDeviceMetrics]
