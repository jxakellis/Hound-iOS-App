...
enum InternalRequestUtils {
    ...
    private static func genericRequestFailureResponse(forRequest request: URLRequest, invokeErrorManager: Bool, completionHandler: @escaping ([String: Any]?, ResponseStatus) -> Void, forResponseBody responseBody: [String: Any]) {
        ...
        DispatchQueue.main.async {
            ...
            if responseError.name == ErrorConstant.PermissionResponseError.noUser.name || responseError.name == ErrorConstant.PermissionResponseError.noFamily.name {
            ...
            else if responseError.name == ErrorConstant.FamilyResponseError.deletedDog.name ||
